...
int prvX,prvY,buttonState,selectedObject,mode;
...
void DoMouse(int button,int state,int x,int y)
{
    ...
    if(state==GLUT_DOWN)
    {
        ...
        case KIST_MOUSE_LEFT:
            ...
            selectedObject=SelectObject(camera.xAxis*(x-(GLfloat)WindowWidth/2)/(WindowWidth/2)-camera.yAxis*(y-(GLfloat)WindowHeight/2)/(WindowHeight/2)+camera.normal,camera.position);
            break;
...
void DoMotion(int x,int y)
{
    ...
    {
    case KIST_MOUSE_LEFT:
        if(selectedObject>=0)aObject[selectedObject].translation=aObject[selectedObject].translation+(camera.xAxis*(GLfloat)(x-prvX)/(WindowWidth/2)*2-camera.yAxis*(GLfloat)(y-prvY)/(WindowHeight/2)*2)/scale;
        break;
    case KIST_MOUSE_RIGHT:
